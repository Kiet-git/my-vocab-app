...
type Answer = {
  wordId: string; correctAnswer: string; userAnswer: string;
  isCorrect: boolean; timeTakenMs: number; distractors: string[];
};
...
// SM-2 helper
function sm2(current: { ease_factor?: number; repetitions?: number; interval_days?: number } | null, quality: 0|1|2|3|4|5) {
  ...
  const interval  = newReps <= 1 ? 1 : newReps === 2 ? 6 : Math.round((current?.interval_days ?? 1) * newEF);
  const status    = newReps >= 5 ? "mastered" : newReps >= 2 ? "reviewing" : newReps >= 1 ? "learning" : "new";
...
export default function QuizPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const load = useCallback(async () => {
    // Single query: topic + words together via join
    ...
    const { data: ws } = await supabase
      .from("words")
      .select("id,topic_id,term,language,definition,pronunciation,example_sentence,icon,difficulty,sort_order,status,notes,image_url,audio_url,ai_generated,ai_model,created_by,created_at,updated_at,word_count")
      .eq("topic_id", topic.id)
      .eq("status", "published")
      .order("sort_order");

    if (!ws || ws.length < 2) { setLoading(false); return; }
  ...
  const pct     = total > 0 ? Math.round(((current + (confirmed ? 1 : 0)) / total) * 100) : 0;
  ...
  const handleNext = async () => {
    ...
    if (current + 1 >= total) {
      ...
      if (session?.user && topicId) {
        ...
        // 2. Save answers + update progress in parallel
        ...
        await Promise.all([
          supabase.from("user_word_progress").upsert(upsertPayload, { onConflict: "user_id,word_id" }),
          supabase.rpc("increment_points", { p_user_id: userId, p_points: finalScore * 10 }),
        ]);
      }
    ...
    <>
      ...
      <div className="bg-mesh min-h-screen flex items-center justify-center pt-20">
        <div className="text-center space-y-4">
          ...
          <p className="text-on-surface-variant text-sm font-medium">Loading quiz...</p>
        </div>
      </div>
    </>
  );
    ...
    <>
      ...
      <div className="bg-mesh min-h-screen flex items-center justify-center pt-20 px-6">
        <div className="text-center space-y-5 max-w-sm">
          <span className="material-symbols-outlined text-6xl text-on-surface-variant/20 block">quiz</span>
          <h2 className="text-2xl font-bold font-headline">Not enough words</h2>
          <p className="text-on-surface-variant">Add at least 2 published words to this topic first.</p>
              ...
              <div className="bg-surface-container-lowest rounded-[1.9rem] p-10 space-y-6 text-center">
                <div className="w-24 h-24 bg-tertiary-container/30 rounded-full flex items-center justify-center mx-auto">
                  <span className="material-symbols-outlined text-tertiary text-5xl"
                    ...
                    {finalPct >= 80 ? "workspace_premium" : finalPct >= 50 ? "auto_awesome" : "sentiment_satisfied"}
                  </span>
                </div>
                <div>
                  <h2 className="text-4xl font-extrabold font-headline">
                    {finalPct >= 80 ? "Excellent!" : finalPct >= 50 ? "Good Job!" : "Keep Going!"}
                  </h2>
                  <p className="text-on-surface-variant mt-2">
                    <span className="text-primary font-bold">{finalScore}/{total}</span> correct
                    {" · "}
                    <span className="text-tertiary font-bold">+{finalScore * 10} pts</span>
                  </p>
                </div>
        ...
        <div className="max-w-2xl mx-auto space-y-6 pt-6">
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 flex-1">
              <span className="text-sm font-bold text-on-surface-variant whitespace-nowrap">{current + 1}/{total}</span>
          ...
          <div className="glass-card rounded-[2rem] p-8 md:p-10 text-center space-y-3 shadow-xl">
            ...
            <p className="text-xs font-bold text-primary tracking-[0.2em] uppercase">{q.language}</p>
            <h2 className="text-4xl md:text-5xl font-extrabold font-headline text-on-surface">{q.term}</h2>
            {q.pronunciation && (
              <p className="text-on-surface-variant text-sm italic">{q.pronunciation}</p>
            )}
            {q.example_sentence && (
          ...
          <div className="space-y-2.5">
            {options.map((opt, idx) => {
              ...
              let cls = "bg-surface-container-lowest border-outline-variant/20 hover:border-primary/40 hover:bg-primary-container/5 cursor-pointer";
              if (confirmed) {
                if (isCorrectOpt)               cls = "bg-tertiary-container/25 border-tertiary cursor-default";
                else if (isSelected)            cls = "bg-error-container/20 border-error/60 opacity-80 cursor-default";
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <span className="leading-relaxed text-sm md:text-base">{opt}</span>
                  </div>
                </button>
              );
            })}
          ...
          {!confirmed ? (
            ...
            <div className={`rounded-2xl p-4 flex items-center justify-between gap-4 ${correct ? "bg-tertiary-container/20" : "bg-error-container/10"}`}>
              <div className="flex items-center gap-3 min-w-0">
                <span className={`material-symbols-outlined text-2xl shrink-0 ${correct ? "text-tertiary" : "text-error"}`}
                  style={{ fontVariationSettings: "'FILL' 1" }}>
                  {correct ? "check_circle" : "cancel"}
                </span>
                <div className="min-w-0">
                  <p className={`font-bold ${correct ? "text-tertiary" : "text-error"}`}>
                    {correct ? "Correct! +10 pts" : "Not quite."}
                  </p>
                  {!correct && (
                    <p className="text-xs text-on-surface-variant truncate mt-0.5">
                      Answer: {q.definition.slice(0, 55)}{q.definition.length > 55 ? "…" : ""}
                    </p>
                  )}
                </div>
              </div>
