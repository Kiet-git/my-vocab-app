...
export async function getTopicsWithProgress() {
  ...
  if (!user) return topics.map((t) => ({ ...t, words_seen: 0, words_mastered: 0 }));
...
export async function getWordsWithProgress(topicId: string) {
  ...
  if (error) throw error;
  if (!user) return words.map((w) => ({ ...w, learning_status: "new" as LearningStatus }));
...
export async function updateWordProgress(
  ...
  const newEF       = Math.max(1.3, ef + 0.1 - (5 - responseQuality) * (0.08 + (5 - responseQuality) * 0.02));
  const newReps     = responseQuality >= 3 ? reps + 1 : 0;
  const newInterval = newReps <= 1 ? 1 : newReps === 2 ? 6 : Math.round((current?.interval_days ?? 1) * newEF);
  const nextReview  = new Date(Date.now() + newInterval * 86400000).toISOString();

  const newStatus: LearningStatus =
    newReps >= 5 ? "mastered" :
    newReps >= 2 ? "reviewing" :
    newReps >= 1 ? "learning" : "new";
...
export async function updateDailyStreak(wordsStudied: number, quizzesDone: number, pointsEarned: number) {
  const supabase = await createServerSupabaseClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) throw new Error("Unauthenticated");
  ...
  const goalReached = wordsStudied >= (profile?.daily_goal ?? 10);
  ...
  if (goalReached) {
    ...
    const newStreak = yesterdayLog?.goal_reached ? (profile2?.current_streak ?? 0) + 1 : 1;
    const longestStreak = Math.max(newStreak, profile2?.longest_streak ?? 0);
...
export async function createGenerateWordsJob(topicId: string, options: {
  count:      number;
